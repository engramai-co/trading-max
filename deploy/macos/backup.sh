...
if [[ ! "$RETAIN_ARCHIVES" =~ ^[1-9][0-9]*$ ]]; then
  echo "TRADING_MAX_BACKUP_RETAIN must be a positive integer" >&2
  exit 64
fi
...
"$BACKUP_PYTHON" - "$STATE_ROOT" "$DESTINATION" <<'PY'
import sys
from pathlib import Path

state, destination = (Path(value).resolve() for value in sys.argv[1:])
