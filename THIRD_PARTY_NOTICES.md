...
| [Alpaca Market Data](https://docs.alpaca.markets/us/docs/about-market-data-api) | Optional historical US SIP and BOATS bars for account reconstruction | Disabled by default; uses user-supplied credentials and only read-only market-data GET endpoints. Basic historical access is delayed. Cached responses stay outside Git and are not redistributed. Trading Max is not affiliated with Alpaca. |
...
| Parqet logo assets and Google favicon service | Best-effort company marks in the interface | Requests are proxied and bounded by the local server; cached files remain outside Git. Logos are decorative and never block financial data. Standalone cached logo assets are not redistributed; documentation screenshots may show company marks in their product context. |
