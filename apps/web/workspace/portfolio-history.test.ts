...
describe("unified portfolio history", () => {
  ...
  it.each(["6M", "YTD", "1Y", "ALL"] as const)("keeps %s on daily resolution", (range) => {
    const result = selectPortfolioHistory({ ...input, range });
    expect(result.source).toBe("daily");
    expect(result.points.every((p) => !p.intraday)).toBe(true);
  });
...
describe("visible observation gaps", () => {
  it.each([["1D", 10], ["1W", 30], ["1M", 60], ["3M", 120]] as const)("uses fixed %s display buckets without changing ten-minute records", (range, minutes) => {
    ...
    const history = selectPortfolioHistory({ ...input, range, asOf: records.at(-1)!.date, intraday: records });
    ...
    const expected = [...new Set([0, ...Array.from({ length: 144 / stride }, (_, index) => (index + 1) * stride - 1)])];
  ...
  it("preserves missing slots and source boundaries through display reduction", () => {
    ...
    expect(series.some((p) => p.value[0] === 42 && p.value[1] === null)).toBe(true);
