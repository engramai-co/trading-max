...
describe("sampled trading-time axis", () => {
  ...
  it("bounds a real collection gap with null values so hovering does not fabricate coverage", () => {
    ...
    expect(series.map((p) => p.value)).toEqual([[0, 100], [1, 101], [2, null], [5, null], [6, 106], [7, 107]]);
  });
});
